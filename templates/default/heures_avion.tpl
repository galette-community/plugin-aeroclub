<form action="heures_avion.php" method="post" class="tabbed">
    <div id="prefs_tabs">
        <ul>
            <li>
                <a href="#tableau">
                    {_T string="HEURES AVION.TABLEAU"}
                </a>
            </li>
            <li>
                <a href="#mois12">
                    {_T string="HEURES AVION.MOIS12"}
                </a>
            </li>
            <li>
                <a href="#selection">
                    {_T string="HEURES AVION.ANNEES"}
                </a>
            </li>
        </ul>
        <fieldset id="tableau" class="cssform">
            ...
            <table class="listing">
                <thead>
                    <tr>
                        <th>
                            <a href="?tri=avion&direction={if $tri eq 'avion' && $direction eq 'asc'}desc{else}asc{/if}">
                                {_T string="HEURES.AVION"}
                            </a>
                            {if $tri eq 'avion' && $direction eq 'asc'}
                                <img src="{$template_subdir}images/down.png"/>
                            {elseif $tri eq 'avion' && $direction eq 'desc'}
                ...
                <tbody>
                    {foreach from=$avions item=av name=avion}
                        <tr>
                            <td class="tbl_line_{if $smarty.foreach.avion.index is even}even{else}odd{/if}">
                                {$av->avion}
                            </td>
                            <td class="tbl_line_{if $smarty.foreach.avion.index is even}even{else}odd{/if}" align="right">
                                {$av->somme_last_year}
                            </td>
                            <td class="tbl_line_{if $smarty.foreach.avion.index is even}even{else}odd{/if}" align="right">
                                {$av->somme_this_year}
        ...
        <fieldset id="selection" class="cssform">
            ...
            <p style="padding-left: 10px">
                ...
                <label for="immat_tous">{_T string="SOLDES PILOTES.TOUS"}</label
            </p>
            <p style="padding-left: 10px">
            ...
            <center>
                <input type="submit" id="zoom" value="{_T string="GRAPHIQUE.AFFICHER"}">
            </center>
            <p style="padding-left: 10px">
