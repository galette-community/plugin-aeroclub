...
for ($i = 0; $i < count($avion); $i++) {
    ...
    for ($m = -11; $m <= 0; $m++) {
        $stats[$avion[$i]][$m] = $stats[$avion[$i]][$m - 1] + PiloteOperation::getDureeVolPourAvion($avion[$i], intval(date('m', strtotime($m . ' months'))), intval(date('Y', strtotime($m . ' months'))));
...
header("Content-Disposition:inline ; filename=heures_avion_cumul.png");
